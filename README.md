...
## FIRST!
in root directory run these commands:
- yarn install
- anchor build (a keypain will be generated in target/deploy)
Change declare_id in lib.rs with the public key of the keypair in target/deploy/keypair.json. And also change the public key in anchor.toml with the same public key.

## Dont mix these two json! :
Wallet / upgrade authority / payer: your ~/.config/solana/id.json
Program id: the public key derived from target/deploy/<program>-keypair.json  (when you run "anchor build", it is automatically created)
...
`anchor test --skip-local-validator` -> this does not work. try the below command instead:
`anchor test --skip-local-validator --skip-deploy` -> since surfpool already deployed your program(surfpool start) you should skip deploy.
